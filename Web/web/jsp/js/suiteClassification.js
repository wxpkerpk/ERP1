...
$(function () {
    $('#tt').treegrid({
        ...
        lines: true,
        fit: true,
        url: '../type.controller/getNodeForSuiteClassification.controller',
        toolbar: [{
            iconCls: "icon-add",
            text: "添加类型",
            handler: addDept
...
function addDept() {
    parent.$("#add").window({
        ...
        width: 600,
        height: 500,
        content: '<iframe src="../jsp/addSuiteClassfication.jsp" frameborder="0" width="100%" height="100%"/>'
    });
}
...
function updateDept() {
    ...
    if (rows.length != 1) {
        ...
        parent.$("#add").window({
            ...
            width: 600,
            height: 500,
            content: '<iframe src="../jsp/updateSuiteType.jsp" frameborder="0" width="100%" height="100%"/>'
        });
    }
}
            ...
            if (r) {
                ...
                $.post('../type.controller/delete.controller', {parentId: ids}, function (result) {
                    if (result.msg == "ok") {
                        ...
                        if (parent.$('iframe[title="套房风格管理"]').get(0)) {
                            parent.$('iframe[title="套房风格管理"]').get(0).contentWindow.$("#tt").treegrid('reload');
                        }
                        if (parent.$('iframe[title="图层分类管理"]').get(0)) {
                            parent.$('iframe[title="图层分类管理"]').get(0).contentWindow.$("#tt").treegrid('reload');
                        }
                        if (parent.$('iframe[title="图层分类管理"]').get(0)) {
                            parent.$('iframe[title="图层分类管理"]').get(0).contentWindow.$("#tt").datagrid('reload');
